...
bool ScalarConverter::argCount(int argc) {
	if (argc < 2)
		return std::cout << NOARG << std::endl, false;
	if (argc > 2)
		return std::cout << TOOMARG << std::endl, false;
	return true;
}
...
bool ScalarConverter::parser(std::string &input) {
	if (input.size() == 0)
		return false;
	if (ScalarConverter::containsOnlySpace(input))
		return false;
	if (ScalarConverter::hasInvalidSpace(input))
		return false;
	std::string trimmedInput = trimSpace(input, " ");
	if (ScalarConverter::isInf(trimmedInput))
		return true;
	if (ScalarConverter::isValidNumber(trimmedInput))
		return true;
	if (ScalarConverter::isValidCharString(trimmedInput))
		return true;
	return false;
}
...
bool ScalarConverter::hasInvalidSpace(std::string &input) {
	...
	if (trimmedString.find(" ") != std::string::npos)
		return true;
	return false;
}
...
bool ScalarConverter::isInf(std::string &input) {
	...
	for (int infType = 0; infType < 8; infType++)
		if (input.compare(inf[infType]) == 0)
			return type = INF, true;
	return false;
}
